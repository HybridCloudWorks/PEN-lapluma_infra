...
VALID_RULE_TYPES = frozenset({"regex", "range", "cross_field", "custom_bounded"})
...
class BlueprintValidator:
    ...
    def _builtin_schema_check(self, data: dict[str, Any], src: str) -> list[str]:
        ...
        allowed_keys = {
            ...
            "source", "fields", "validationRules", "evidenceRequirements", "syntheticFixtures"
        }
        for key in data:
    ...
    def _evaluate_synthetic_fixtures(self, data: dict[str, Any], src: str) -> list[str]:
        ...
        for idx, fixture in enumerate(fixtures):
            ...
            # Evaluate validity of input_values against required fields and rules
            is_valid, reason = self._check_fixture_validity(input_values, fields, rules)
    ...
    def _check_fixture_validity(
        ...
        for path, field in fields.items():
            if field.get("required", False):
                if path not in input_values or input_values[path] is None or input_values[path] == "":
        ...
        for rule in rules:
            ...
            if rtype == "regex":
                # Expression format: "canonicalPath:regex_pattern"
                parts = expr.split(":", 1)
                if len(parts) == 2:
                ...
                # Expression format: "canonicalPath:min:max"
                parts = expr.split(":")
                if len(parts) == 3:
                ...
                # Expression format: "fieldA==fieldB" or "fieldA!=fieldB"
                if "==" in expr:
