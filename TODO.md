...
## Approved GCP delivery index — 2026-09-13

The owner approved the [GCP target](wiki/GCP-Document-Library-Decision.md). The existing phases below
remain historical/current-scaffold context; conflicting Azure-only tasks are superseded by the
linked GCP cards and must not initiate Azure provisioning. Preserve applicable isolation, approval,
retention, quality and operational requirements. GitHub Projects is the live status/acceptance
record for this new cross-repository scope, as requested by the owner.

All entries below have **Status: Todo** at publication. **Recommended action:** implement the linked
card after its dependencies and attach acceptance evidence. **Notes for future engineers:** the
approved target is not deployed; retain existing safeguards until their replacements pass checks.

| Title | Priority | Description | Dependencies |
|---|---|---|---|
| [INF-01: Reconcile every USCIS form into the Document Library](https://github.com/orgs/HybridCloudWorks/projects/2) | P0 | Delivered official USCIS manifest schema (`contracts/schemas/uscis-manifest.schema.json`), authoritative reconciled inventory (`contracts/uscis-official-manifest.json`) covering all 117 USCIS forms across I-series (91), N-series (10), G-series (14), and other prefixes (2: AR-11, EOIR-29), with complete preparation capability dispositions (`FILLABLE_PDF`: 104, `STATIC_ASSISTED`: 10, `EXTERNAL_REFERENCE`: 3) and 100% epic assignments (`INF-06`, `INF-07`, `INF-08`). Preserved non-USCIS definitions (`DS-11`, `FAFSA`, `CLINIC-INTAKE`, `SCHOLARSHIP-APP`), CLI validator (`tools/uscis_manifest_validator.py`), unit tests (`tools/test_uscis_manifest.py`), and foundation validation integration. | None |
| [INF-02: Define PostgreSQL Blueprint and Collection data contracts](https://github.com/orgs/HybridCloudWorks/projects/2) | P0 | Delivered PostgreSQL DDL in `infra/sql/001_document_library_schema.sql` and `002_workflow_persistence_schema.sql` plus C# contract models (`DocumentLibraryModels.cs`, `WorkflowBlueprintModels.cs`). Relational keys for tenants, blueprints, collections, cases, approvals, and upload sessions; indexed JSONB for fields schema, validation rules, and evidence requirements. | INF-15; INT-01 |
| [INF-03: Acquire official artifacts through the GCP publication pipeline](https://github.com/orgs/HybridCloudWorks/projects/2) | P1 | Replace Azure acquisition bindings with Cloud Scheduler-triggered Cloud Run jobs and Pub/Sub events. Implement constrained official-source fetching, redirects/type/size validation, hashes, source provenance, retries and isolated Cloud Storage staging. Acquisition creates candidates and never publishes an edition. | INF-01; INF-02; INF-16 |
| [INF-04: Publish immutable Blueprint revisions with independent review](https://github.com/orgs/HybridCloudWorks/projects/2) | P0 | Delivered publication lifecycle DDL (`infra/sql/004_blueprint_publication_lifecycle.sql`) enforcing independent review (`reviewer != author`), source checksum and manifest pinning, automatic edition drift quarantine, and non-destructive case rollback; Core API publication services (`IBlueprintPublicationService.cs`, `PostgresBlueprintPublicationService.cs`, `InMemoryBlueprintPublicationService.cs`) and endpoints; CLI lifecycle commands in `tools/blueprint_cli.py`; and automated tests (`BlueprintPublicationLifecycleTests.cs`, `tools/test_blueprint_cli.py`). | INF-02; INF-17; INT-14 |
| [INF-05: Implement declarative Blueprint validation and PDF mapping](https://github.com/orgs/HybridCloudWorks/projects/2) | P0 | Extend existing field schemas into data-only Blueprints with namespaced canonical paths, conditional/repeated sections, mappings, evidence requirements and preparation capability. First release supports original fillable PDFs, static documents with assisted preparation, and external reference workflows. Institution customization cannot alter official content or platform authorization. | INF-02; INF-17 |
| [INF-06: Complete I-series form and supplement coverage](https://github.com/orgs/HybridCloudWorks/projects/2) | P1 | Create one child implementation task per I-series entry and supplement from INF-01, with named engineering owner, edition, schema, artifact, capability, dependencies and fixture. Include existing I-130/I-130A, I-485/I-864 and I-765 package contracts in regression coverage. Deliver these I-series entries through the shared Blueprint onboarding/publication process; do not create a service per form. Shared official sources remain unmodified. | INF-01; INF-04; INF-05 |
| [INF-07: Complete N-series form and supplement coverage](https://github.com/orgs/HybridCloudWorks/projects/2) | P1 | Create per-form child tasks for all N-series entries and supplements in the verified manifest; cover citizenship/naturalization forms and role-restricted portions using official instructions. Deliver these N-series entries through the shared Blueprint onboarding/publication process; do not create a service per form. Shared official sources remain unmodified. | INF-01; INF-04; INF-05 |
| [INF-08: Complete G-series, other series and external-form coverage](https://github.com/orgs/HybridCloudWorks/projects/2) | P1 | Create per-form child tasks for every remaining catalog entry, including G-series and any other prefixes discovered by INF-01. Include fee/reference documents, restricted workflows, online-only entries and supplements without inventing fillable PDFs. Deliver these G-series and every remaining series entries through the shared Blueprint onboarding/publication process; do not create a service per form. Shared official sources remain unmodified. | INF-01; INF-04; INF-05 |
| [INF-09: Version official and institutional document guidance](https://github.com/orgs/HybridCloudWorks/projects/2) | P1 | Store source-cited, revision-bound instructions, evidence requirements and fee references. Keep issuer requirements separate from institution guidance. Fees are optional where irrelevant; unknown values are not guessed. Institution notes and cover documents cannot override official requirements. | INF-02; INF-03 |
| [INF-10: Implement durable PostgreSQL catalog and workflow persistence](https://github.com/orgs/HybridCloudWorks/projects/2) | P0 | Delivered Npgsql PostgreSQL 16 persistence for Core API catalog & library access (`PostgresCatalogSource.cs`, `PostgresLibraryAccessService.cs`), Workflow API client folders, per-person trust boundaries, case workspaces, idempotency keys, and transactional outbox (`PostgresWorkflowSource.cs`), DDL idempotency constraints (`002_workflow_persistence_schema.sql`), and contract test suites (`PostgresCatalogContractTests.cs`, `PostgresWorkflowPersistenceTests.cs`). | INF-02; INT-02 |
| [INF-11: Connect GCP document processing and approved output services](https://github.com/orgs/HybridCloudWorks/projects/2) | P1 | Wire Cloud Run processing and batch jobs to private Cloud Storage objects, Pub/Sub and Document AI. Keep hostile parsing isolated from the database and authoritative stores. Core services accept anchored proposals, preserve human confirmation, and generate edition-pinned official outputs only after approval. | INF-05; INF-10; INT-05; INT-06 |
| [INF-12: Specify grayscale styling for any GCP operator surfaces](https://github.com/orgs/HybridCloudWorks/projects/2) | P1 | Inventory infra-owned operator surfaces; none is established in the reviewed source tree. Preserve the DESIGN.md-derived grayscale specification for any such surface. The first Blueprint onboarding and publication interface is CLI/CI, so no new administration frontend is required. | INT-10 |
| [INF-13: Apply grayscale operator UX only to established frontends](https://github.com/orgs/HybridCloudWorks/projects/2) | P2 | Apply the grayscale specification to established or separately approved infra frontends for edition review, acquisition, publication and audit. Do not create a new frontend solely for styling or initial customer onboarding. | INF-12; INF-04 |
| [INF-14: Gate library coverage and operational readiness](https://github.com/orgs/HybridCloudWorks/projects/2) | P1 | Validate complete USCIS manifest reconciliation separately from Blueprint preparation capability. Gate publication on per-revision validation, source freshness and consumer compatibility. Add Pub/Sub dead-letter, acquisition, processing and storage lifecycle recovery evidence. | INF-06; INF-07; INF-08; INF-09; INT-11 |
| [INF-15: Adopt lean GCP architecture and replace superseded Azure decisions](https://github.com/orgs/HybridCloudWorks/projects/2) | P0 | Baseline recorded and approved: Cloud Run APIs/workers, Cloud SQL PostgreSQL (db-g1-small pilot tier), private Cloud Storage objects with lifecycle tiering, usage-based Pub/Sub, API Gateway, and platform encryption. Supersedes Azure/HSM/Cosmos baseline. | None |
| [INF-16: Build Terraform and federated delivery foundation for GCP](https://github.com/orgs/HybridCloudWorks/projects/2) | P0 | Delivered modular Terraform in `infra/terraform/` covering IAM (GitHub Actions Workload Identity Federation, least-privilege SAs), VPC network (Private Services Access for Cloud SQL, Direct VPC Egress, zero Cloud NAT cost), Cloud Storage (artifacts, documents, scratch with lifecycle rules), Cloud SQL PostgreSQL 16 (`db-g1-small` with Secret Manager credentials), Pub/Sub (topics, subscriptions, DLQ), Cloud Run v2 (scale-to-zero compute), and Google API Gateway across `dev`, `staging`, and `pilot` environments. | INF-15 |
| [INF-17: Create reusable Document Blueprint onboarding template and CLI](https://github.com/orgs/HybridCloudWorks/projects/2) | P0 | Delivered declarative JSON schema (`contracts/schemas/document-blueprint.schema.json`), operator onboarding & validation CLI (`tools/blueprint_cli.py` with `init`, `validate`, `package`), sample blueprints across fillable PDF, static assisted, and external reference modes (`blueprints/`), automated suite (`tools/test_blueprint_cli.py`), and integrated foundation validation gate. | INF-02 |
| [INF-18: Implement customer Collections and library access controls](https://github.com/orgs/HybridCloudWorks/projects/2) | P0 | Delivered PostgreSQL access control views & multi-tenant seed fixtures (`infra/sql/003_library_access_views_and_seeds.sql`), domain service (`ILibraryAccessService.cs`, `LibraryAccessService.cs`), server-side claims tenant resolution (`TenantResolution.cs`), `/v1/library` collections & blueprints endpoints, and comprehensive tenant isolation test suite (`LibraryAccessControlTests.cs`). | INF-02; INT-02 |
| [INF-19: Measure per-institution usage and validate pilot/growth costs](https://github.com/orgs/HybridCloudWorks/projects/2) | P1 | Record usage by environment, institution and operation without PII: API calls, compute time, OCR/structured extraction pages, model tokens, stored bytes and transfers. Produce pilot/growth estimates including dev/staging, backup/log retention and document processing; add alerts and scale/concurrency ceilings. Allocate shared overhead separately from marginal usage. | INF-16; INF-10 |
| [INT-02: Map application identity and authorization to GCP](https://github.com/orgs/HybridCloudWorks/projects/4) | P0 | Delivered GCP IAM & Workload Identity Federation configuration, dedicated API Gateway service account (`gateway_sa`), Cloud Run invoker restrictions (`roles/run.invoker` granted strictly to API Gateway SA; direct public invocation denied), API Gateway edge OIDC validation (`x-google-issuer`, `x-google-audiences`, `jwt_audience`), PostgreSQL separation-of-duty DDL (`infra/sql/005_database_roles_and_permissions.sql`) enforcing least-privilege roles (`lapluma_app_core`, `lapluma_app_workflow`, `lapluma_library_admin`), and verification suites (`DatabaseRoleSeparationTests.cs`, `validate_foundation.py`). | INT-01 |
| [INT-05: Complete scoped Cloud Storage upload-to-extraction transfer](https://github.com/orgs/HybridCloudWorks/projects/4) | P1 | Delivered direct-to-storage document ingestion under ADR-019. 100 MB uploads (`104,857,600` bytes) bypass Google API Gateway's 32 MB limit via short-lived (15-minute), narrowly scoped create-only grants (V4 signed PUT URLs) to private Cloud Storage staging/quarantine buckets (`storage.googleapis.com`). Server validates actual file size, SHA-256 checksum, tenant ownership, and MIME type before completion. Fail-closed typed problem details (422) on digest/size mismatches, replay absorption, and zero token leakage. Added `GoogleCloudStorageUploadUrlIssuer`, `IStoredDocumentVerifier`, Terraform `quarantine_bucket` with 24h auto-expiry, and comprehensive test suite (`GoogleCloudStorageUploadSessionTests.cs`). | INF-10; INT-02; INF-16 |

## Earlier scaffold backlog

### Windows checkout contract-byte preservation

- **Priority:** P2
- **Description:** Git CRLF conversion changes the bytes of the hash-pinned
  `contracts/openapi/workforce-workflow.yaml`, causing foundation validation and five tool tests
  to fail in an otherwise unchanged Windows checkout.
- **Dependencies:** None.
- **Recommended action:** Declare the mirrored contract's required LF bytes in Git attributes and
  verify fresh Windows/Linux checkouts without changing the adopted contract hash.
- **Status:** Open; discovered 2026-09-13.
- **Notes for future engineers:** Restoring the exact `git show HEAD:<path>` bytes locally made
  foundation validation and all 99 Python tests pass. Do not adopt a new hash to accommodate CRLF.
