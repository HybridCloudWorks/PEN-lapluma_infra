...
|---|---|---|---|
| [INF-01: Reconcile every USCIS form into the Document Library](https://github.com/orgs/HybridCloudWorks/projects/2) | P0 | Create the verified, versioned manifest for every USCIS all-forms entry and supplement, capturing issuer, official title, source URL, verification time, revision and preparation capability. USCIS is the first major library expansion, not the platform schema. Preserve existing non-USCIS definitions. | None |
| [INF-02: Define PostgreSQL Blueprint and Collection data contracts](https://github.com/orgs/HybridCloudWorks/projects/2) | P0 | Model versioned Document Blueprints, Collections and institution assignments in PostgreSQL. Use relational keys, foreign keys and transactions for identity, ownership, grants, cases and approvals; JSONB for variable field definitions and bounded declarative rules. Blueprint identity includes issuer/owner namespace and immutable revision; an official edition date is optional metadata, not a universal identity requirement. | INF-15; INT-01 |
...
| [INF-04: Publish immutable Blueprint revisions with independent review](https://github.com/orgs/HybridCloudWorks/projects/2) | P0 | Implement draft, validation, independent review and publication for Blueprints and Collections. Publications pin source hashes, mappings and revisions. Add withdrawal, edition drift, affected-case quarantine and rollback; published revisions are immutable. LaPluma staff perform publication initially. | INF-02; INF-17; INT-14 |
...
| [INF-10: Implement durable PostgreSQL catalog and workflow persistence](https://github.com/orgs/HybridCloudWorks/projects/2) | P0 | Replace Microsoft.Data.SqlClient/T-SQL and fixture-only workflow storage with PostgreSQL persistence for catalog, tenants, cases, canonical values, upload sessions, idempotency, outbox and approvals. Use transactions, relational constraints and tenant/person-scoped row security with non-bypass runtime roles. Keep reads in PostgreSQL with measured indexes/materialized views only if needed. | INF-02; INT-02 |
...
| [INF-15: Adopt lean GCP architecture and replace superseded Azure decisions](https://github.com/orgs/HybridCloudWorks/projects/2) | P0 | Record the approved baseline: Cloud Run APIs/workers and Jobs, Cloud SQL PostgreSQL as the only database, JSONB definitions, private Cloud Storage objects, Pub/Sub, API Gateway, Scheduler, Artifact Registry, platform encryption and TLS. Preserve separate processing/service identities. Retire Azure/HSM/CMEK/Cosmos/premium-broker assumptions from current ADRs, checklists and planned CI rather than deleting historical decisions. | None |
| [INF-16: Build Terraform and federated delivery foundation for GCP](https://github.com/orgs/HybridCloudWorks/projects/2) | P0 | Prepare modular Terraform for the selected GCP services, isolated environments, least-privilege service accounts, GitHub workload identity federation, private SQL, processing egress restrictions, scoped Cloud Storage access and logging. Use no service-account key files, dedicated HSM, Cosmos/Firestore or dedicated message broker. Use private Google service access where needed without granting processors database routes. | INF-15 |
| [INF-17: Create reusable Document Blueprint onboarding template and CLI](https://github.com/orgs/HybridCloudWorks/projects/2) | P0 | Create a declarative schema, example directory, CLI and CI validation workflow for LaPluma operators. Template includes issuer/owner and namespace, rights/source provenance, immutable source hash/revision, artifact type, fields/mappings, evidence, bounded rules, capability, access scope, localization and synthetic fixtures. Support fillable PDF, static assisted document and external reference in v1; other renderers require explicit capability work. | INF-02 |
| [INF-18: Implement customer Collections and library access controls](https://github.com/orgs/HybridCloudWorks/projects/2) | P0 | Persist ordered, revision-pinned Blueprint references, institution guidance/branding and customer assignments in PostgreSQL. Resolve effective access server-side from tenant membership and assignments; never accept client tenant selection as authorization. Shared Blueprints are immutable; private variants have explicit ownership and revision lineage. | INF-02; INT-02 |
